...
class Model:
    ...
    def __init__(self, datafile = "C:\ML\Train2.csv", model_type = None):
        ...
        """X = np.array(self.df[['Gender', 'Married','Education','Self_Employed','ApplicantIncome','LoanAmount','Loan_Amount_Term','Credit_History','Property_Area']])
        Y = np.array(self.df['Loan_Status'])     
        self.X=X
        self.Y=Y
        from sklearn.impute import SimpleImputer   #check null values
        si=SimpleImputer(strategy='most_frequent');
        X[:,0:6]=si.fit_transform(X[:,0:6])
        X[:,8:10]=si.fit_transform(X[:,8:10])
        si1=SimpleImputer(strategy='mean');
        X[:,5:8]=si1.fit_transform(X[:,5:8])
        for i in range(0,9):
            if  i==0 or i==1 or i==2 or i==3 or i==8 :
